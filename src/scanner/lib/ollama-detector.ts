...
import type { DetectionResult, FlaggedSentence } from '../types'
import { detectLanguageHeuristic, type HeuristicResult } from './language-heuristic'
...
function buildUserPrompt(text: string, locale: string, excludedTerms: string[]): string {
  const exclusions =
...
function parseLLMResponse(content: string, sourceText: string): FlaggedSentence[] {
    ...
    parsed.examples
      .filter(
        (s: { text?: unknown; englishWords?: unknown }) =>
          typeof s.text === 'string' && Array.isArray(s.englishWords) && s.englishWords.length > 0,
      )
...
export async function detectEnglishWithLLM(
    ...
    return examples
      .map((s) => {
        ...
        return {
          text: s.text,
          englishWords: s.englishWords.filter(
            (w) => !singleWordExcluded.has(w.toLowerCase()) && !multiMatched.has(w.toLowerCase()),
          ),
        }
      })
  ...
  const heuristicExamples: FlaggedSentence[] = heuristic.flaggedSentences.map((s) => ({
    text: s.text,
  ...
  try {
    const llmExamples = await fetchExamplesFromLLM(text, locale, model, excludedTerms)

    // Merge: heuristic first, then LLM extras not already found
    const seenTexts = new Set(heuristicExamples.map((e) => e.text.toLowerCase()))
    ...
    const untranslatedPercent =
      filteredMerged.length > 0 ? Math.max(recalculatePercent(filteredMerged), 1) : 0
